...
**Semaine 5 — réseau.** Cinq specs dans `cypress/e2e/network/`, douze tests, pour les cas que le backend ne produit jamais. Le rendement n'est pas le nombre de tests : c'est **trois défauts de l'application** qu'aucun test contre le vrai backend ne pouvait atteindre. Un 500 s'affiche comme une liste vide — `dataMachine` entre bien en `failure` avec un message, aucun composant ne le lit ; une coupure réseau donne exactement le même écran ; et un montant négatif injecté dans une réponse réelle se rend `--$5.00`, parce que le composant préfixe un `-` que le formateur produit déjà. Les specs constatent ces défauts au lieu d'affirmer un comportement que l'application n'a pas. S'y ajoute un risque, constaté et **volontairement non testé** : `asyncUtils.ts:3` crée axios **sans `timeout`**, et axios vaut `timeout: 0` par défaut — sa doc dit « a timeout is not created ». Un backend lent laisse donc la liste en chargement sans fin, sans que l'application puisse l'interrompre. L'E2E qui le prouvait coûtait 9 s pour une propriété statique du code — retiré au nom du même principe qui interdit les E2E inutiles (P3), et documenté à la place.
