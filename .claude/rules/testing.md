...
12. **État XState** : lire `window.__services__` (garde `VITE_TEST_HOOKS`) via une app action de `support/app-actions/`, jamais `cy.window()` inline dans une spec. `window.authService` reste disponible sous garde `window.Cypress` (ADR-006).
