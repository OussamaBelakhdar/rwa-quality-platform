...
12. **État XState** : lire `window.__services__` (garde `process.env.VITE_TEST_HOOKS`, figé au build) via une app action de `support/app-actions/`, jamais `cy.window()` inline dans une spec. Une app action **attend** qu'un service apparaisse : les services portés par un composant n'existent que monté (ADR-006).
