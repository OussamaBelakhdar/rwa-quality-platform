...
9. **Sélecteurs** : `cy.getBySel(key)` avec `key: DataTestKey`, ou `cy.findByRole` (Testing Library). Un `data-test` écrit en dur dans un `cy.get` est bloqué par `check-spec.sh` — sinon la garantie de typage s'arrête au premier développeur qui contourne la commande. Ajouter un `data-test` dans `src/` est autorisé et doit mettre à jour `selectors/data-test.ts`.
