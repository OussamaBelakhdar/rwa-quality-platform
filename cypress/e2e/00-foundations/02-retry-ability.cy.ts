// Niveau E2E : la retry-ability se démontre sur un DOM qui se peuple vraiment
// après un appel réseau. Un composant monté avec des props figées ne la montre pas.
...
describe("Fondations — retry-ability", { tags: ["@foundations", "@regression"] }, () => {
  ...
  it("réessaie la dernière query jusqu'à ce que l'assertion passe", () => {
    // Aucune attente explicite : `should` relance `getBySelLike` jusqu'au succès
    // ou jusqu'au timeout. C'est ce qui rend cy.wait(<nombre>) inutile (P4).
