 ...
 * cette porte, `cy.env()` garde la valeur côté runner (ADR-001, révision
 * « la frontière est ternaire »).
 *
 * Le type est volontairement optionnel : `cy.env<T>` est un générique non
 * vérifié à l'exécution. Le déclarer `string` ferait croire au compilateur que
 * la garde ci-dessous est morte.
 *
