...
| **Allowed Callback URLs** et **Allowed Web Origins** = `http://localhost:3000` | Application → Settings                  | l'application envoie `redirect_uri: window.location.origin` (`src/utils/auth0Options.ts:27`), donc `http://localhost:3000`. Sans la première, Auth0 refuse la redirection ; sans la seconde, l'échange du `code` depuis le SPA est bloqué par CORS — le même symptôme que celui rencontré sur le fournisseur local, neuf `OPTIONS` et zéro `POST`                                                |
...
| « Non-Verifiable Callback URI End-User Confirmation » → **Skip**               | Application → Advanced Settings → OAuth | `http://localhost:3000` est une URI de **bouclage**, donc non vérifiable : Auth0 interpose alors une confirmation contre l'usurpation d'application sur la même machine. « Allow Skipping User Consent » ne la supprime pas, et son balisage n'est pas documenté — la désactiver vaut mieux que deviner un sélecteur. Le réglage existe aussi au niveau du tenant ; celui de l'application prime |
...
**Trois créations et un réglage.** Créer une application **SPA**, une **API**
dont l'Identifier devient l'`audience`, un **utilisateur de test** — puis
désactiver la confirmation de callback non vérifiable sur cette application.
Les quatre autres lignes de ce tableau ont été supprimées par du code ou par
la révision de la décision — pas par de la documentation.

Auth0 déconseille de désactiver cette confirmation, et il a raison — pour une
application de production servie sur un vrai domaine. Ici l'URI de bouclage
n'existe que parce que la suite tourne en local, et le réglage ne touche que
l'application de test. La recommandation vise un risque que ce contexte n'a pas.
