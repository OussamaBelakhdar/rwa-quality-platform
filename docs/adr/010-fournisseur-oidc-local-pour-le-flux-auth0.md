...
| Le code d'autorisation est à **usage unique** — un rejeu échoue                               | Qu'un `client_id`, une `audience` ou un `scope` **non enregistrés** soient refusés : le serveur local réémet ce qu'il reçoit, là où un tenant vérifie contre son registre |
