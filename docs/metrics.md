...
| Tests                         | **54** (40 → 54)                     | 14 tests réseau                                                                                                                                                                                      |
| Suite complète                | vert, **49 s**                       | 54/54                                                                                                                                                                                                |
| `cypress/e2e/network/` seul   | **18 s**, 14/14                      | 3 exécutions consécutives vertes                                                                                                                                                                     |
| `yarn cy:random`              | vert, **8 ordres sur 8**, 48-50 s    | preuve d'isolation (P1) maintenue. Deux mesures écartées : voir ci-dessous                                                                                                                           |
| `yarn cy:burn` sur `network/` | **10 × 14 = 140 exécutions, 0,00 %** | retries forcés à zéro ; seuil §6 : 2 %                                                                                                                                                               |
...
| Sélecteurs `data-test`        | 75 → **78**                          | 3 clés ajoutées par l'écran d'erreur, `yarn check:selectors` et `check:autocompletion` verts                                                                                                         |
| Correctifs dans `src/`        | **4 défauts, 6 fichiers**            | écran d'erreur (`TransactionList` + 3 listes), message d'erreur perdu (`dataMachine`), double signe et montant nul (`TransactionAmount`). Chacun verrouillé par un test de régression mutation-testé |

### Ce que le stub réseau a trouvé, que le backend ne pouvait pas produire

C'est le rendement de la semaine : neuf comportements que la suite ne pouvait
pas atteindre avant. **Sept sont corrigés ici** — cinq dans l'application, deux
dans l'outillage de test. Les deux restants, un risque et une contrainte de
conception, sont documentés sans être traités, et l'ADR dit pourquoi.

| Constat                                                              | Preuve                                                                                                                                                                                                                        | Statut                                                                                                                                                      |
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Un **500 se rendait comme une liste vide** — pas de message d'erreur | `dataMachine` entrait bien en `failure`, mais aucun composant ne le lisait : le rendu retombait sur `showEmptyList` (`TransactionList.tsx`)                                                                                   | **corrigé dans l'application** — écran d'erreur dédié avec message et bouton de reprise ; `showEmptyList` exclut désormais `hasError`                       |
...
Les sept premiers ne sont atteignables que par intercept : le backend réel ne
