...
| Tests                         | **57** (40 → 57)                     | 17 tests réseau, sur 6 fichiers                                                                                                                                                                                                                         |
| Suite complète                | vert, **32 s**                       | 57/57. Les 48-50 s relevés plus tôt l'étaient avec des tâches de fond concurrentes — cf. « mesures écartées »                                                                                                                                           |
| `cypress/e2e/network/` seul   | **13 s**, 17/17                      | 3 exécutions consécutives vertes                                                                                                                                                                                                                        |
| `yarn cy:random`              | vert, **9 ordres sur 9**             | preuve d'isolation (P1) maintenue. Deux mesures écartées : voir ci-dessous                                                                                                                                                                              |
| `yarn cy:burn` sur `network/` | **10 × 17 = 170 exécutions, 0,00 %** | retries forcés à zéro ; seuil §6 : 2 %                                                                                                                                                                                                                  |
...
| Sélecteurs `data-test`        | 75 → **78**                          | 3 clés ajoutées par l'écran d'erreur, partagées par les 4 surfaces, `yarn check:selectors` et `check:autocompletion` verts                                                                                                                              |
| Correctifs dans `src/`        | **7 défauts, 11 fichiers**           | écran d'erreur partagé (`ErrorState` + 4 surfaces `dataMachine`), message d'erreur perdu (`dataMachine`), chargement invisible du détail, double signe et montant nul (`TransactionAmount`). Chacun verrouillé par un test de régression mutation-testé |
